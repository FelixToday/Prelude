...
from Prelude_main.Run.utils_dataset_metric import load_data
...
def get_parser(mode="train"):
    ...
    parser.add_argument('--machine_name', type=str, default=get_machine_name(),
                        help='服务器名称')
    parser.add_argument('--config', default="config/Prelude.ini",
                        help="模型配置文件路径")
    ...
    # 模型相关
    parser.add_argument('--TAM_type', type=str, default='ED1',
                        help="提取特征的TAM方法")
    ...
    # ========== 增强方式参数 ==========
    parser.add_argument('--early_stage', type=str_to_bool, default=False,
                        help="是否启用 early_stage 增强（Loss 层随机采样 L 维位置）")
    parser.add_argument('--early_augment', type=str_to_bool, default=False,
                        help="是否启用 early_augment 增强（输入层随机截断）")
    parser.add_argument('--aug_lb', type=float, default=0.01,
                        help="early_augment 截断比例下界")
    parser.add_argument('--aug_ub', type=float, default=1.0,
                        help="early_augment 截断比例上界")
    parser.add_argument('--aug_num', type=int, default=20,
                        help="增强版本数量（early_augment / early_stage 每次生成的随机位置数）")
    # ===================================

    # 测试模式相关
    parser.add_argument('--is_Sen', type=str_to_bool, default=False,
                        help="是否为参数敏感性测试模式")
    parser.add_argument("--Sen_aug_num", type=int, default=30)
...
def prepare_batch_data(cur_data, device):
    ...
    # 处理 early_augment 带来的额外 k 维: (B, k, C, H, W) -> (B*k, C, H, W)
    if cur_X.dim() == 5:
        B, K, C, H, W = cur_X.shape
        cur_X = cur_X.reshape(B * K, C, H, W)
        cur_y = cur_y.reshape(B * K)
...
def adjust_system_args(args, config):
    ...
    if args['is_Sen']:
        ...
        config['aug_num'] = args['Sen_aug_num']

    # ========== 增强方式校验 ==========
    if args.get('early_stage', False) and args.get('early_augment', False):
        raise ValueError("early_stage 和 early_augment 不能同时为 True！请只用一种增强方式或都不启用。")
    # ===================================
...
def truncate_traffic_by_ratio_v2(X_raw, load_ratio):
    """
    按加载比例截断原始流量数据 —— 将截断点之后的数据包置零。
    
    Args:
        X_raw: numpy array, shape (N, L, 2) — 原始流量，列0=时间戳，列1=包大小
        load_ratio: float 或 numpy array, [0, 100] — 加载百分比
    
    Returns:
        X_truncated: numpy array, shape (N, L, 2) — 截断后的数据，超出部分置零
    """
    X_truncated = X_raw.copy()
    N = len(X_truncated)
    
    if np.isscalar(load_ratio):
        load_ratio = np.full(N, load_ratio)
    
    for i in range(N):
        timestamp = np.trim_zeros(X_truncated[i, :, 0], "b")
        if len(timestamp) == 0:
            continue
        max_time = timestamp[-1]
        threshold = max_time * (load_ratio[i] / 100.0)
        cutoff_idx = np.searchsorted(timestamp, threshold, side='right')
        if cutoff_idx < len(X_truncated[i]):
            X_truncated[i, cutoff_idx:, :] = 0.0
    
    return X_truncated


def truncate_traffic_by_time_v2(X_raw, max_time):
    """
    按绝对时间截断原始流量数据 —— 将超过 max_time 的数据包置零。
    
    Args:
        X_raw: numpy array, shape (N, L, 2)
        max_time: float — 最大观测时间（秒）
    
    Returns:
        X_truncated: numpy array, shape (N, L, 2)
    """
    X_truncated = X_raw.copy()
    N = len(X_truncated)
    
    for i in range(N):
        timestamp = X_truncated[i, :, 0]
        cutoff_idx = np.searchsorted(timestamp, max_time, side='right')
        if cutoff_idx < len(X_truncated[i]):
            X_truncated[i, cutoff_idx:, :] = 0.0
...
def get_truncation_index_v2(timestamp, max_trace_time, current_ratio=None, current_windows=None,
                            w_const=80.0/1800.0, win_step=6, mode='ratio'):
    """
    通用截断索引计算 —— 支持 ratio 和 window 两种步进模式。
    
    Args:
        timestamp: numpy array — 有效时间戳序列
        max_trace_time: float — 当前 trace 的总时长
        current_ratio: float — 当前比例 (仅 ratio 模式)
        current_windows: int — 当前窗口索引 (仅 window 模式)
        w_const: float — 窗口时间常数 (maximum_load_time / max_matrix_len)
        win_step: int — 窗口步进步长
        mode: str — 'ratio' 或 'window'
    
    Returns:
        cutoff_idx: int — 截断位置索引
        current_ratio: float — 实际的当前比例 [0, 1]
    """
    if mode == 'ratio':
        actual_ratio = min(current_ratio, 1.0)
    elif mode == 'window':
        actual_ratio = min((current_windows * w_const) / max_trace_time, 1.0)
    else:
        raise ValueError(f"不支持的mode: {mode}，请使用 'ratio' 或 'window'")
    
    threshold = actual_ratio * max_trace_time
    cutoff_idx = np.searchsorted(timestamp, threshold, side='right')
    
    return cutoff_idx, actual_ratio
