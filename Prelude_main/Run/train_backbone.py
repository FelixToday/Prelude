# -*- coding: utf-8 -*-
"""
Phase 1: Backbone 训练脚本
替代原 main.py，支持 early_stage / early_augment 增强方式。
"""
...
def train_one_epoch(model, train_loader, criterion, optimizer, device, config, args, epoch, scheduler=None, miner=None):
    ...
    for index, cur_data in enumerate(tqdm(train_loader)):
        ...
        # 损失计算
        if args.get('early_stage', False):
            # Early Stage: Loss 层在 L 维上随机采样 k 个位置计算 loss
            N, L, D = outs.shape
            noise = torch.rand(N, L, device=outs.device)
            ids_shuffle = torch.argsort(noise, dim=1)
            ids_keep = ids_shuffle[:, :args.get('aug_num', 30)]
            outs_masked = torch.gather(outs, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))

            cur_y_expanded = cur_y.unsqueeze(1).repeat(1, args.get('aug_num', 30))
...
def main():
    ...
    if args.get('early_stage', False) and args.get('early_augment', False):
        raise ValueError("early_stage 和 early_augment 不能同时为 True")
    # ===================================
...
if __name__ == "__main__":
    main()
    print_colored("train_backbone 全部运行结束", "green")
