...
def get_model(num_classes, num_tabs, patch_size, **kwargs):
    """实例化 Prelude 模型"""
    model = Prelude(num_classes=num_classes, num_tabs=num_tabs, patch_size=patch_size, **kwargs)
    return model


if __name__ == '__main__':
    from torchinfo import summary
    model = get_model(num_classes=95, num_tabs=5, patch_size=5, max_matrix_len=7200)
    data = torch.rand((20, 1, 5, 7200))
    model(data)
    summary(model, input_size=(1, 1, 5, 7200), depth=float('5'),
