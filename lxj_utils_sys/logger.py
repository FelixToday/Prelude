...
class BaseLogger_v2:
    def __init__(self, json_path: Optional[str] = None, log_path: Optional[str] = None, is_console_out: bool = True):
        """
        轻量级实验日志记录器
    ...
    def print(self, msg: str, *args, save_to_file: bool = False, level: str = "INFO", **kwargs):
        """
        ...
        log_entry = self._log_fmt % {
            'asctime': datetime.now().strftime(self._date_fmt)[:-3],
            'filename': os.path.basename(frame.f_code.co_filename),
    ...
    def record(self, key: str, value: Any, overwrite: bool = False, unpack_dict: bool = False) -> None:
        ...
        # 更新时间并保存
        self.data['meta']['updated_at'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self._save_json()
