...
class CountDataset(Dataset):
    def __init__(self, X, labels, args, BAPM=None, is_idx=False):
        self.X = X
        self.labels = labels
...
if __name__ == "__main__":
    import numpy as np
    # 定义数据集路径
    base_dir = r"G:\0 用户存储数据\学习记录\7科研\论文\website figerprinting attack\代码\baseline\COUNTMAMBA\CountMamba-WF-main\dataset\CW"
    train_path = f"{base_dir}\\train.npz"
    train_data = np.load(train_path)
    X_train, y_train = train_data['X'], train_data['y']
    a = CountDataset(X_train, y_train)
