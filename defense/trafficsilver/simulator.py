...
parser.add_argument("-s", "--scheme", type=str, help="Splitting scheme", default='in_and_out')
parser.add_argument("-c", "--cells", type=int, help="Cells per circuit in RR (in others it is the initial value)",
                    default=1)
...
parser.add_argument("-o", "--outfolder", type=str, help="Folder for output files",
                    default='../results/trafficsilver_rb_DF/')
...
parser.add_argument("-r", "--ranges", type=str,
                    help="Range of cells after of which the wr or wrwc schduler design again", default='50,70')
parser.add_argument("-p", '--path', default="../../dataset/DF/",
                    help=' fiPath of folder with instancesles (wang_format)')
parser.add_argument("-a", "--alpha", type=str, help="alpha values for the Dirichlet function default np.ones(m)",
                    default='1,1,1')

schemes = ['round_robin', 'random', 'weighted_random', 'in_and_out', 'batched_weighted_random', 'bwr_var_paths',
           'bwr_var_paths_strict', 'bwr_blocked', 'wr_var_paths', 'rr_var_paths', 'random_var_paths']
bwoh = [0]
...
def saveInFile(input_name, inst, r, outfolder):
    ...
    for k in range(0, numberOfFiles):
        input_name2 = input_name.split('.cell')[0].split(os.sep)[-1]
        out_file_name = outfolder + "/" + input_name2 + "_split_" + str(k) + '.cell'
        outfiles.append(open(out_file_name, 'w'))

    jointfilename = outfolder + "/" + input_name.split('.cell')[0].split(os.sep)[-1] + "_join" + '.cell'
...
def saveInFile2(input_name, split_inst, r, outfolder):
    ...
    if "." in os.path.basename(input_name):
        appendix = "." + input_name.split(".")[-1]
        base_name = os.path.basename(input_name).split(".")[0]
    else:
        appendix = ""
    ...
    outfiles = []
    for k in range(0, numberOfFiles):
        out_file_name = outfolder + base_name + "_split_" + str(k) + appendix
        outfiles.append(open(out_file_name, 'w'))

    # jointfilename = outfolder + base_name + "_join" + appendix
    # jointfile = open(jointfilename, 'w')
    ...
    for i in range(0, len(split_inst)):
        ...
        strwrt = str(x_arrstr[0] - ts_o[rout]) + '\t' + str(x_arrstr[1] * x_arrstr[2])
...
def sim_random(n, latencies, traces, outfiles):
    ...
    for instance_file in traces_file:
        ...
        mplatencies = getCircuitLatencies(latencies,
                                          n)  # it is a list of list of latencies for each of m circuits. length = m
        routes = [random.randrange(0, n, 1) for _ in range(len(instance))]  # Random routes
        new_instance = multipath.simulate(instance, mplatencies,
                                          routes)  # Simulate the multipath effect for the given latencies and routes
        saveInFile(instance_file, new_instance, routes,
                   outfiles)  # Save the transformed current instance according to thei respective routes
...
def sim_round_robin(n, latencies, traces, outfiles, cpercirc):
    ...
    for instance_file in tqdm.tqdm(traces_file):
        try:
            with open(instance_file, 'r', encoding='utf-8') as f:
                instance = f.read().split('\n')[:-1]
        except (UnicodeDecodeError, ValueError):
            print("Skipping non-trace file: %s" % instance_file)
            continue
        # Get n circuits latencies as a multipath virtual structure of the same client for this instance
        mplatencies = getCircuitLatencies(latencies,
                                          n)  # it is a list of list of latencies for each of m circuits. length = m
        ...
        new_instance = multipath.simulate(instance, mplatencies,
                                          routes)  # Simulate the multipath effect for the given latencies and routes
        saveInFile2(instance_file, new_instance, routes,
                    outfiles)  # Save the transformed current instance according to thei respective routes
...
def sim_in_and_out(n, latencies, traces, outfiles):
    ...
    for instance_file in tqdm.tqdm(traces_file):
        ...
        for i in range(0, len(instance)):
            ...
            if (
                    direction == -1):  # if it is incomming just sent through a random route, it'd better trying to have same amount of
                routes_server.append(1)  # Just to know that for this packet the exit does not decide the route
...
def sim_bwr_var_paths_strict(n, nmin, latencies, traces, outfiles, range_):
    ...
    print(
        "Simulating bwr with variable number of paths strict to avoid less than nmin Cmax and Cmind should be reduced to 15-20")
    traces_file = natsorted(glob.glob(traces[0] + '/*.cell'))
    ranlow = int(range_.split(',')[0])
    ranhigh = int(range_.split(',')[1])
...
def sim_wr_var_paths(n, nmin, latencies, traces, outfiles):
    ...
    for instance_file in traces_file:
        ...
        # Get n circuits latencies as a multipath virtual structure of the same client for this instance
        mplatencies = getCircuitLatencies(latencies,
                                          n_random)  # it is a list of list of latencies for each of m circuits. length = m
        # for each instance I need to create a new randomly created weights vector
        routes_server = []
        routes_client = []
        last_client_route = np.random.choice(np.arange(0, n_random), p=w_out)
...
def sim_rr_var_paths(n, nmin, latencies, traces, outfiles, cpercirc):
    ...
    for instance_file in traces_file:
        ...
        new_instance = multipath.simulate(instance, mplatencies,
                                          routes)  # Simulate the multipath effect for the given latencies and routes
        saveInFile2(instance_file, new_instance, routes,
                    outfiles)  # Save the transformed current instance according to their respective routes
