...
def getDirfromPacket(packet): return int(packet.split('\t')[1]) // abs(int(packet.split('\t')[1]))


def getSizefromPacket(packet):
    return abs(int(packet.split('\t')[1]))
...
def getWeights(n, alphas):
    ...
    if (len(aph) != n):
        return -1
    vec_alphas = np.array(aph, dtype=np.float64)
...
def simulate(instance, mplatencies, routes):
    ...
    for i in range(0, len(instance)):  # Iterate over each packet
        ...
        if (last_packet != direction): delay = float(
            original_time - last_time) / 2  # Calculate the RTT/2 (latency) request/response, from the time the out cell is sent till the correspongin incell arrives
        #######################################################################################################################################################
        ...
        if (
                direction == 1 and last_packet == -1):  # If is the first out in the burst, it referes to the last icomming time
    ...
    sorted_new_trace = np_new_trace[np_new_trace[:,
                                    0].argsort()]  # Sorted according to the new timestamps, this is the final instance applied the multi-path effects
